		...
		api.ajax({
			...
			success: xhr => {
				...
				for (var i = 0; i < xhr.logs.length; i++) {
					s += '<tr><td [[w1]]>' + xhr.logs[i].id + '</td><td [[w2]]>' + new Date(xhr.logs[i].createdAt.replace('+00:00', '')).toLocaleString().replace(' ', '&nbsp;') + '</td><td [[w3]]>' + xhr.logs[i].status + '</td><td [[w4]]>' + (xhr.logs[i].ip ? '<a href="https://whatismyipaddress.com/ip/' + xhr.logs[i].ip + '" target="sc_ip">' + xhr.logs[i].ip + '</a>' : '') + '</td><td [[w5]]>' + xhr.logs[i].method + ' ' + xhr.logs[i].uri + (xhr.logs[i].query ? '?' + xhr.logs[i].query : '') + '</td><td [[w6]]>' + xhr.logs[i].time + '</td>';
					if (!narrowView)
